# Stage 4: GenBank export

Paste the section below into `R/annotation_map.qmd` after the annotation map.
The output `.gbk` file goes to `results/`, which is gitignored, so the
nucleotide content is never published. Only the rendered HTML in `docs/`
goes to GitHub Pages.

The `primer_bind` features are derived from the e-PCR amplicon coordinates:
the forward primer occupies the first N bases of the amplicon (where N is
the forward primer length), the reverse primer occupies the last M bases.
...
if (exists("amp_hits") && nrow(amp_hits) > 0) {
  for (j in seq_len(nrow(amp_hits))) {
    ...
    # forward primer at the 5' end of the amplicon (relative to amplicon strand)
    if (amp_strand == "+") {
